...
# shellcheck disable=SC2016,SC2188
<<COMMENT
...
oc delete --ignore-not-found csv "$CSV" -n "$NGINX_OPERATOR_NS"
oc delete --ignore-not-found csv "$CSV" -n "$NGINX_APPS_NS"
oc delete --ignore-not-found namespace "$NGINX_OPERATOR_NS"
oc delete --ignore-not-found namespace "$NGINX_APPS_NS"
