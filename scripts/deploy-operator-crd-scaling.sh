...
if [[ "$(uname -s)" == "Darwin"* ]]; then
  log "Skipping operator crd scaling install on MacOS"
  exit 0
fi
...
log "Cloning crd operator version ${CHECKOUT_FOLDER}."
...
log "Deploying operator with container image version ${CRD_SCALING_TAG}"
...
log "Waiting for deployment jack to appear until $(date --date=@"${END_TIME}")"
...
while [[ $(date -u +%s) -le $END_TIME ]]; do
  ...
  log "Deployment jack not found yet. Waiting 5 secs..."
  sleep 5
done

if ! ${DEPLOYMENT_FOUND} ; then
	log_err "Timeout waiting for operator's deployment."
	exit 1
fi

# Wait until jack deployment's pods are ready.
log "Waiting ${TIMEOUT_MINS}m for operator's deployment jack in namespace ${TNF_EXAMPLE_CNF_NAMESPACE} to be ready."
if ! oc wait deployment jack \
	    --for=condition=available \
	    --namespace "$TNF_EXAMPLE_CNF_NAMESPACE" \
	    --timeout="${TIMEOUT_MINS}"m ; then
  log_err "Operator's deployment jack is not ready."
  exit 1
fi
...
log "Removing crd operator checkout folder ${CHECKOUT_FOLDER}"
rm -rf "${CHECKOUT_FOLDER}"
