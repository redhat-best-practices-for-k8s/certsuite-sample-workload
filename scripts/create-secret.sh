...
if [[ -n "$(ls "$SCRIPT_DIR"/certs/registry.pem 2>/dev/null)" ]]; then
	...
	oc delete secret "$SECRET_NAME" -n "$CERTSUITE_EXAMPLE_NAMESPACE" --ignore-not-found=true
	oc create secret generic "$SECRET_NAME" --from-file="$SCRIPT_DIR"/certs/cert.pem -n "$CERTSUITE_EXAMPLE_NAMESPACE"
else
