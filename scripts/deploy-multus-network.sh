...
if $CERTSUITE_NON_OCP_CLUSTER; then
	...
	git clone $WHEREABOUTS_GIT_URL --depth 1 -b v0.9.0

	# sed replace whereabouts:latest with whereabouts:v0.9.0
	sed 's/whereabouts:latest/whereabouts:v0.9.0/g' whereabouts/doc/crds/daemonset-install.yaml -i
